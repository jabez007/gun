(function() {
    ...
    SEA.secret =
      ...
      (async (key, pair, cb, opt) => {
        try {
          ...
          var props = Object.assign(S.ecdh, {
  ...
  USE(function(module) {
    ...
    // let's extend the gun chain with a `group` function.
    // the user can only be 'logged in' to one group at a time, per gun instance.
    Gun.chain.group = function(pub) {
      var gun = this,
        root = gun.back(-1),
        group;
      if (pub) {
      ...
      if ((group = root.back("group"))) {
        return group;
      }
      // dictates how to generate universally unique identifiers for objects created under a group
      var root = root._,
        at = root,
        uuid = at.opt.uuid || Gun.state.lex;
      (at = (group = at.group = gun.chain(new Group()))._).opt = {};
      at.opt.uuid = function(cb) {
        var id = uuid(),
          upub = root.user,
          gpub = root.group;
        ...
        // the user and group that own the obect with be in the uuid of the object
        id = id + "~" + upub + "%" + gpub + ".";
        if (cb && cb.call) {
          cb(null, id);
        }
  ...
  USE(function(module) {
    ...
    User.prototype.auth = function(alias, pass, cb, opt) {
      ...
      act.g = function(pair) {
        ...
        at.sea = act.pair;
        // prevent 'group-bleed' upon successful login
        if ((group || {}).is) {
          // has group been instantiated, and is there already a group set?
          group.leave();
        }
        cat.ing = false;
        try {
    ...
    User.prototype.pubs = function(alias, cb) {
      var gun = this,
        root = gun.back(-1),
        cb =
          cb ||
    ...
    // **** Group ***
    Group.prototype.create = function(alias, cb, opt) {
      var gun = this,
        cat = gun._,
        root = gun.back(-1),
        user = root._.user;
      ...
      if (!(user || {}).is) {
        cb({
          err: Gun.log("Must be logged in to create a group!"),
          wait: true
        });
        return gun;
      }
      if (cat.ing) {
        cb({
          err: Gun.log("Group is already being created or joined!"),
          wait: true
        });
        return gun;
      }
      ...
      grp.a = function(pubs) {
        ...
        if (pubs && !opt.already) {
          // If we can enforce that a user name is already taken, it might be nice to try, but this is not guaranteed.
          var ack = { err: Gun.log("Group already created!") };
          cat.ing = false;
          cb(ack);
          gun.leave();
          return;
        ...
        // no salt needed?
        // group's private key will be encrypted under each user that belongs to the group
        // this does create a 'vulnerability' if any of the user accounts are compromised
        grp.b();
      };
      grp.b = function() {
        SEA.pair(grp.c); // now we have generated a brand new ECDSA key pair for the user account.
      };
      grp.c = function(pair) {
        var tmp;
        grp.pair = pair || {};
        if ((tmp = cat.root.group)) {
          tmp._.sea = pair;
          tmp.is = { pub: pair.pub, epub: pair.epub, alias: alias };
        }
        // the groups's public key doesn't need to be signed. But everything else needs to be signed with it!
        grp.data = { pub: pair.pub };
        grp.d();
      };
      grp.d = function() {
        grp.data.alias = alias;
        SEA.secret(user.is.epub, grp.pair, grp.e); // Derive shared secret from user's epub and group's epub/epriv
        // later on the group's epub can be used only with the user's epub/epriv to re-derive the shared secret
        // salt the secret since it is essentially a "password" for the group?
      };
      grp.e = function(secret) {
        grp.data.epub = grp.pair.epub;
        SEA.encrypt(
          { priv: grp.pair.priv, epriv: grp.pair.epriv },
          secret,
          grp.f,
          { raw: 1 }
        ); // to keep the private key safe, we encrypt it with the shared secret!
        // encrypt-ception?
      };
      grp.f = function(auth) {
        var users = {};
        users[user.is.pub] = auth;
        grp.data.users = JSON.stringify(users); // store the list of users that are a part of the group in the group. Allows other members of the group the remove a user from the authorized list.
        grp.g(grp.data.users);
      };
      grp.g = function(users) {
        var tmp;
        grp.data.users = grp.data.users || users;
        root.get((tmp = "%" + grp.pair.pub)).put(grp.data); // awesome, now we can actually save the group with its public key as its ID.
        root.get("%@" + alias).put(Gun.obj.put({}, tmp, Gun.val.link.ify(tmp))); // next up, we want to associate the alias with the public key. So we add it to the alias list.
        setTimeout(function() {
          // we should be able to delete this now, right?
          cat.ing = false;
          cb({ ok: 0, pub: grp.pair.pub }); // callback that the group has been created. (Note: ok = 0 because we didn't wait for disk to ack)
          if (noop === cb) {
            gun.join(alias);
          } // if no callback is passed, auto-join after creating.
        }, 10);
      };
      root.get("%@" + alias).once(grp.a);
      return gun;
    };
    Group.prototype.pubs = function(alias, cb) {
      var gun = this,
        root = gun.back(-1),
        cb =
          cb ||
    ...
    // now that we have created a group, we want to join it!
    Group.prototype.join = function(alias, cb, opt) {
      var gun = this,
        cat = gun._,
        root = gun.back(-1),
        user = root._.user;
      cb = cb || function() {};
      if (!(user || {}).is) {
        cb({ err: Gun.log("Must be logged in to join a group!"), wait: true });
        return gun;
      }
      if (cat.ing) {
        cb({
      ...
      grp.a = function(data) {
        ...
        if (data.length) {
          // its an array and it is populated
          return grp.b(data);
        }
        if (grp.name) {
          return grp.f(data);
        }
        grp.c((grp.data = data).users);
      };
      grp.b = function(list) {
        var get = (grp.list = (grp.list || []).concat(list || [])).shift(); // removes the first element from an array and returns that element
        if (u === get) {
          if (grp.name) {
            return grp.err(
              "This group is not published for dApps to access, please consider syncing it online, or allowing local access by adding your device as a peer."
            );
          }
          return grp.err("You are not authorized to join this group");
        }
        root.get(get).once(grp.a);
      };
      grp.c = function(users) {
        if (u === users) {
          return grp.b();
        }
        if (Gun.text.is(users)) {
          return grp.c(Gun.obj.ify(users));
        } // in case of legacy
        if (u === (grp.users = users)[user.is.pub]) {
          return grp.b();
        } // user does not exist in the list for group
        SEA.secret(grp.data.epub, user._.sea, grp.d); // derive the shared secret from the group's epub and the user's epub/epriv
      };
      grp.d = function(secret) {
        SEA.decrypt(grp.users[user.is.pub], secret, grp.e, grp.enc);
      };
      grp.e = function(half) {
        if (u === half) {
          // decryption failed
          if (!grp.enc) {
            // try old format
            grp.enc = { encode: "utf8" };
            return grp.c(grp.users);
          }
          grp.enc = null; // end backwards
          return grp.b();
        }
        grp.half = half;
        grp.f(grp.data);
      ...
      grp.g = function(pair) {
        grp.pair = pair;
        var group = root._.group,
          at = group._;
        var tmp = at.tag;
        var upt = at.opt;
        at = group._ = root.get("%" + pair.pub)._;
        at.opt = upt;
        // add our credentials in-memory only to our root group instance
        group.is = { pub: pair.pub, epub: pair.epub, alias: alias };
        at.sea = grp.pair;
        cat.ing = false;
        cb(at);
        try {
          root._.on("join", at); // TODO: Deprecate this, emit on group instead! Update docs when you do.
          //at.on('join', at) // Arrgh, this doesn't work without event "merge" code, but "merge" code causes stack overflow and crashes after logging in & trying to write data.
        } catch (e) {
          Gun.log("Your 'join' callback crashed with:", e);
        }
      };
      grp.err = function(e) {
        var ack = { err: Gun.log(e || "Group cannot be found!") };
        cat.ing = false;
        cb(ack);
      };
      grp.plugin = function(name) {
        if (!(grp.name = name)) {
          return grp.err();
        }
        var tmp = [name];
      ...
      if (alias) {
        // root.get("%@" + alias).once(grp.a);
        gun.pubs(alias, grp.a);
      } else if (!alias) {
        SEA.name(grp.plugin);
      }
      return gun;
    };
    Group.prototype.leave = function(opt, cb) {
      var gun = this,
        group = gun.back(-1)._.group;
      if (group) {
        delete group.is;
        delete group._.is;
        delete group._.sea;
    ...
    Group.prototype.grant = function(to, cb) {
      var gun = this,
        cat = gun._,
        root = gun.back(-1),
        group = root._.group,
      ...
      if (!(user || {}).is) {
        cb({
          err: Gun.log("Must be logged in to grant access to a group!"),
          wait: true
        });
        return gun;
      }
      if (!group.is) {
        cb({
          err: Gun.log("Must join a group to grant access to that group!"),
          wait: true
        });
        return gun;
      }
      // derive a shared secret from the new user's epub and group's epub/epriv
      // encrypt the groups private keys with this shared secret
      // store the encrypted private keys in the group's "users"
      // {to.pub: enc}
      var grp = {},
        act = {},
        u;
      grp.a = function(currUsers) {
        if (Gun.text.is(currUsers)) {
          return grp.a(Gun.obj.ify(currUsers));
        } // in case of legacy
        grp.users = currUsers;
        // right now, just grant access to one user at a time.
        // eventually grant access to multiple users,
        // or a group, or multiple groups at a time.
        // root.get("~@" + to).once(grp.b);
        user.pubs(to, grp.b);
      };
      grp.b = function(data) {
        if (!data) {
        ...
        grp.d((act.data = data).epub);
      };
      grp.c = function(pubs) {
        var get = (grp.list = (grp.list || []).concat(pubs || [])).shift(); // list of public keys to grant access to this group
        if (u === get) {
          return grp.err("No users found to add to this group!");
        }
        root.get(get).once(grp.b);
      };
      grp.d = function(epub) {
        if (u === epub) {
          return grp.c();
        }
        SEA.secret(epub, group._.sea, grp.e); // Derive shared secret from user's epub and group's epub/epriv
      };
      grp.e = function(secret) {
        SEA.encrypt(
          { priv: group._.sea.priv, epriv: group._.sea.epriv },
          secret,
          grp.f,
          { raw: 1 }
        ); // to keep the private key safe, we encrypt it with the shared secret!
      ...
      grp.f = function(auth) {
        grp.users[act.data.pub] = auth;
        gun.get("users").put(JSON.stringify(grp.users));
        cb({ ok: 0, pub: act.data.pub }); // callback that the user has been added to the group. (Note: ok = 0 because we didn't wait for disk to ack)
      };
      gun.get("users").once(grp.a);
      return gun;
    };
    Group.prototype.revoke = function(from, cb) {
      var gun = this,
        cat = gun._,
        root = gun.back(-1),
        group = root._.group,
      ...
      if (!(user || {}).is) {
        cb({
          err: Gun.log("Must be logged in to revoke access to a group!"),
          wait: true
        });
        return gun;
      }
      if (!group.is) {
        cb({
          err: Gun.log("Must join a group to revoke access to that group!"),
          wait: true
        });
        return gun;
      }
      // gets complicated because we need to generate a new pair for the group,
      // and then encrypt the private keys of the new pair
      // for each user still allowed in the group.
      var grp = {},
        act = {},
        u;
      grp.a = function(currUsers) {
        ...
        grp.users = currUsers;
        // right now, just revoke access for one user at a time.
        // eventually revoke access for multiple users,
        // or a group, or multiple groups at a time.
        // root.get("~@" + from).once(grp.b);
        user.pubs(from, grp.b);
      };
      grp.b = function(data) {
        if (!data) {
          return grp.c();
        ...
        grp.d((act.data = data).pub);
      };
      grp.c = function(pubs) {
        var get = (grp.list = (grp.list || []).concat(pubs || [])).shift(); // list of public keys to revoke access for to this group
        if (u === get) {
          return grp.err("No users found to remove from this group!");
        }
        root.get(get).once(grp.b);
      };
      grp.d = function(pub) {
        ...
        delete grp.users[pub]; // delete the user from the list of valid users on the group
        gun.get("users").put(JSON.stringify(grp.users));
        cb({ ok: 0, pub: act.data.pub }); // callback that the user has been removed from the group. (Note: ok = 0 because we didn't wait for disk to ack)
      };
      // now we need to update the keys for the group to make sure the removed user(s) can't break back into the group
      gun.get("users").once(grp.a);
      return gun;
    };
    module.exports = Group;
